...
# 开始遍历簇数量
print(f"开始 K-means 聚类，当前时间: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}")
for n_clusters in range(2, max_clusters + 1):
    ...
    # 计算综合得分 (1 + Silhouette) * Davies-Bouldin
    combined_score = alpha * np.exp(1 + silhouette_avg) + beta * np.exp(-db_score)
    print(f"簇数量: {n_clusters}, Silhouette Score: {silhouette_avg}, Davies-Bouldin Score: {db_score}, Combined Score: {combined_score}")
...
print(f"最小的 Combined Score: {best_combined_score}")
